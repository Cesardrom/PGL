...
register_request = api.model(
    'RegisterRequest',
    {'alias': fields.String(required=False, description='Alias opcional del dispositivo')},
)
...
match_create_response = api.model(
    'MatchCreateResponse',
    {
        ...
        'players': fields.Raw(description='Diccionario de jugadores (device_id -> símbolo)'),
...
waiting_status_response = api.model(
    ...
    {
        'status': fields.String(description='Estado del dispositivo: waiting, matched o idle'),
...
device_status_response = api.model(
    ...
    {
        'connected': fields.Boolean(description='Indica si el dispositivo está conectado'),
